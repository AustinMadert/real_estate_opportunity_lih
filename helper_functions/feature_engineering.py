...
def train_compute(train_df, train_features, train_labels, compute_df, compute_X,
     compute_y, gridsearch_params=None):
    ...
    if gridsearch_params is not None:
        ...
        grid = GridSearchCV(gradboost, gridsearch_params)
...
def boost_dataframe(gridsearch_params=None):        
...
def main():
    response = input('Would you like to skip cleaning and use pickle? ')
    if response == 'y':
